...
object HoodieSpark40CatalystPlanUtils extends HoodieSpark4CatalystPlanUtils {
  ...
  override def applyNewHoodieParquetFileFormatProjection(plan: LogicalPlan): LogicalPlan = {
    plan match {
      ...
      l@LogicalRelation(fs: HadoopFsRelation, _, _, _, _)) if fs.fileFormat.isInstanceOf[NewHoodieParquetFileFormat] && !fs.fileFormat.asInstanceOf[NewHoodieParquetFileFormat].isProjected =>
