    ...
    val stream2 = spark.readStream.format("hudi")
      ...
      .foreachBatch { (batch: Dataset[Row], id: Long) =>
        ...
        batch
          ...
          .withColumn("bcnt", sparkAdapter.createColumnFromExpression(beforeCntExpr)).withColumn("acnt", sparkAdapter.createColumnFromExpression(afterCntExpr))
          .select(
            explode(array(Array(
          ...
          .select(
            ...
            sparkAdapter.createColumnFromExpression(
              Add(sparkAdapter.getExpressionFromColumn(col("sum(cnt)")), If(sparkAdapter.getExpressionFromColumn(isnull(col("population"))), Literal(0), sparkAdapter.getExpressionFromColumn(col("population"))))).as("population"),
            col("max(ts)").as("ts")
          )
          .write.format("hudi")
          .options(commonOptions)
