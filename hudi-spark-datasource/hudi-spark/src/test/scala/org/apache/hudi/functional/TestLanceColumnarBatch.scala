...
class TestLanceColumnarBatch extends HoodieSparkClientTestBase {
  ...
  private def readAndCollect(
    ...
    iter.foreach {
      case batch: ColumnarBatch =>
        batchCount += 1
        // Extract data immediately before the iterator advances and closes the batch
        batch.rowIterator().asScala.foreach { row =>
          val id = row.getInt(0)
          val name = if (row.isNullAt(1)) null else row.getUTF8String(1).toString
          val col2 = if (requiredSchema.fields.length >= 3) {
            val f = requiredSchema.fields(2)
   ...
   * using [[org.apache.spark.sql.execution.vectorized.OnHeapColumnVector]].
   */
  @Test
  def testColumnarPathNullPadsAbsentColumns(): Unit = {
    ...
    reader.read(pf, requiredSchemaWithExtra, new StructType(), HOption.empty(),
      Seq.empty[Filter], storageConf).foreach {
      case batch: ColumnarBatch =>
        batchesSeen += 1
        // 'score' is at index 2; it must be a null column
        batch.rowIterator().asScala.foreach { row =>
  ...
  def testColumnarPathAppendsPartitionVectors(): Unit = {
    ...
    reader.read(pf, baseSchema, partitionSchema, HOption.empty(),
      Seq.empty[Filter], storageConf).foreach {
      case batch: ColumnarBatch =>
        batchesSeen += 1
        ...
        batch.rowIterator().asScala.foreach { row =>
          // Partition column is appended at the last position
  ...
  def testTypeChangeFallsBackToRowPath(): Unit = {
    ...
    val (batches, rowItems, rows) =
      readAndCollect(filePath, requiredSchemaDoubleScore, enableVectorized = true)
